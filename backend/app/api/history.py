...
from app.models import Spec, Iteration, Evaluation
...
async def get_spec_history(
    ...
    ).order_by(Iteration.ts.desc()).limit(limit).all()
    
    # Get evaluations
    evaluations = db.query(Evaluation).filter(
        Evaluation.spec_id == spec_id
    ).order_by(Evaluation.ts.desc()).limit(limit).all()
