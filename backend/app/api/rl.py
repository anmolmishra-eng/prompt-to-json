...
async def rl_feedback(feedback: dict, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    from app.models import RLHFFeedback
...
async def train_rlhf_ep(params: dict, user=Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if len(pairs) < 10:
        ...
        pairs = [
            ("Improve design", {"objects": [{"id": "obj1", "material": "steel"}]}, 
             {"objects": [{"id": "obj1", "material": "aluminum"}]}, "B"),
            ("Improve design", {"objects": [{"id": "obj2", "material": "wood"}]}, 
             {"objects": [{"id": "obj2", "material": "carbon"}]}, "B"),
            ("Improve design", {"objects": [{"id": "obj3", "material": "plastic"}]}, 
             {"objects": [{"id": "obj3", "material": "metal"}]}, "B"),
