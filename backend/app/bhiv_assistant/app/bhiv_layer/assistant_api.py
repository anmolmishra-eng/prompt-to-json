...
class BHIVAssistant:
    ...
    def __init__(self):
        ...
        self.rl_handler = RLFeedbackHandler(
            base_url=str(self.config.ranjeet.base_url),
            api_key=self.config.ranjeet.api_key
        )
    ...
    async def generate_design(self, request: DesignRequest) -> BHIVResponse:
        ...
        try:
            ...
            try:
                ...
                rl_result = await self._call_ranjeet_rl(
                    client,
                    spec_result["spec_json"],
                    request.city
                )
                ...
                if rl_result:
                    confidence = await self.rl_handler.get_confidence_score(
                        spec_result["spec_json"],
                        request.city
                    )
                    if confidence:
    ...
    async def _call_task7_generate(self, client: httpx.AsyncClient, request: DesignRequest) -> Dict:
        ...
        try:
            ...
            raise HTTPException(
                status_code=503,
                detail=f"Task 7 service unavailable: {str(e)}"
            )
...
async def health_check():
    ...
    try:
        response = await client.get(
            f"{getattr(settings, 'SOHAM_URL', 'https://ai-rule-api-w7z5.onrender.com')}/health",
            timeout=5.0
        )
    ...
    try:
        response = await client.get(
            f"{getattr(settings, 'YOTTA_URL', 'https://api.yotta.com')}/health",
            timeout=5.0
        )
