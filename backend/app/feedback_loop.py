...
class FeedbackLoopOrchestrator:
    ...
    def collect_user_feedback(self, user_id: str, spec_id: str, rating: float, notes: str) -> dict:
        ...
        iterations = self.db.query(Iteration).filter(
        ...
        if len(iterations) >= 2:
            for i in range(len(iterations) - 1):
                ...
                feedback_pairs.append({
                    "before": before_iter.iter_id,
                    "after": after_iter.iter_id,
                    "preference": preference
                })
        ...
        return {
            "feedback_id": rlhf_fb.id,
            "pairs_created": len(feedback_pairs),
            "spec_id": spec_id,
            "user_id": user_id
        }
    ...
    def aggregate_feedback(self, lookback_hours: int = 24) -> dict:
        ...
        recent_evals = self.db.query(Evaluation).filter(
            Evaluation.ts >= cutoff_time
        ).all()
        
        # Get recent RLHF feedback
        recent_feedback = self.db.query(RLHFFeedback).filter(
            RLHFFeedback.timestamp >= cutoff_time
        ).all()
        
        # Calculate statistics
        total_feedback = len(recent_evals) + len(recent_feedback)
        avg_rating = (
            sum(e.score for e in recent_evals if e.score) / len(recent_evals)
...
class IterativeFeedbackCycle:
    ...
    async def process_evaluation_feedback(
        ...
        # 1. Collect feedback
        feedback_result = self.orchestrator.collect_user_feedback(
            user_id, spec_id, rating, notes
        )
